...
> 💡 (2025.01.11 추가) 우리 서비스내에 HA(High Availability)를 위한 아키텍처 개선을 위해, 데이터베이스 레플리케이션 도입 외에도 정말 많은 고민을 했다. 무중단 배포 도입기를 이번 포스팅의 후반부에서 추가적으로 간략히 다루었으니 참고하자. 또는 [Jenkins와 Nginx를 활용한 Blue/Green 무중단 배포 도입기](https://kakaotech-harmony.netlify.app/backend/blue-green/) 에서 별도의 포스팅으로 더욱 자세히 다루었으니, 이를 참고하자.
...
백문이 불여일타. 어떤 문제 상황인지 간단히 설명을 듣는 것 보다, 직접 코드를 어떻게 구현했는지 이해하는 것이 중요하다. 지금부터 우리 팀이 어떻게 코드를 구현했는지 자세히 알아보고, 어떤 해결과정을 거쳤는지 살펴보자.
...
## 🤔 아직 해결하지 못한 추가 고민사항 (WAS 서버로 인한 서비스 다운타임)

위와 같은 트러블슈팅을 해결을 통해 Multi DataSource 기반 분산 DBMS 환경을 정상적으로 구축했으며, 데이터베이스 서버의 가용성(Availability) 를 개선할 수 있게 되었다. 하지만, 우리 서비스 아키텍처에는 아직 추가 개선사항이 남아있다. **데이터베이스와 달리 우리 스프링부트 WAS 서버에서 간혈적으로 서비스 다운타임(Downtime) 이 발생하고 있다는 점이다.**

Jenkins 를 사용하여 배포 자동화 파이프라인을 구성해놓았는데, 구버전 프로세스를 종료하고 신버전 프로세스로 교체하는 그 사이에 일시적으로 서비스 다운타임이 발생한다. **즉, 우리 서비스는 신버전 WAS 서버를 배포할 때 마다 간혈적으로 서비스가 중단되고 있으며, 이 문제점은 곧 유저 이탈로 이어지게 될 위험이 생긴다.** 우리 모행 서비스 유저들에게 더욱 만족스러운 사용자 경험을 제공하기 위해선 다운타임을 제거하고, 가용성을 확보해야 할 것이다.

현재 스프링 WAS 서버의 구버전 프로세스를 종료하고 신버전 프로세스를 배포할 때 발생하는 서비스 다운타임은 어떻게 제거해야할지 고민을 계속 해봤지만, 아직까지는 해결법이 떠오르지 않는다. 이에 대해서는 조금 더 깊은 공부가 필요할 것 같다.

## (2025.01.11 추가) 서비스 다운타임 개선을 위한 무중단 배포 전략 도입

> 💡 (2025.01.11 추가) 우리 모행 팀의 무중단 배포 도입기를 [Jenkins와 Nginx를 활용한 Blue/Green 무중단 배포 도입기](https://kakaotech-harmony.netlify.app/backend/blue-green/) 에서 자세히 다루었으니, 이를 참고하자.

![alt text](image-6.png)

(2025.01.11 추가) 위 문제를 해결하기 위한 방법으로 **무중단 배포(Zero-Downtime Deployment) 전략** 이 있다는 것을 알게 되었다. 데이터베이스 서버 가용성을 개선을 위한 방법 중 하나로 레플리케이션을 도입하였듯이, WAS 서버에서 발생하는 서비스 다운타임을 최소화하기 위한 방법 중 하나가 바로 무중단 배포 전략이다.

기존에 우리가 구성한 Jenkins 내의 배포 파이프라인 스크립트 구성에는 신버전 프로세스를 배포하는 과정에서 문제가 있다는 것을 알게 되었는데, 이와 관련한 자세한 내용을 [Jenkins와 Nginx를 활용한 Blue/Green 무중단 배포 도입기](https://kakaotech-harmony.netlify.app/backend/blue-green/) 에서 다루었으니 참고하자.
