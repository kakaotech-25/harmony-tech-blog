...
Jmeter 툴을 사용하여 TPS 측정 결과를 실제로 확인하였는데, 우선 트랜잭션 밖으로 외부 API 요청을 분리하지 않았을 떄의 결과는 아래와 같이 약 `34.1/sec` 가 측정되었다.
...
하지만 기대와 달리, 트랜잭션을 분리했을 떄와 하지 않았을 떄의 TPS 측정 결과가 거의 동일하게 `33.9/sec` 로 도출된 결과를 확인할 수 있다. 왜 이렇게 동일한 결과가 도출된 것일까? 🤔
...
동일한 성능 테스트 환경에서 외부 API 요청을 트랜잭션에서 분리했을 떄와 안했을 떄의 TPS 처리량을 확인했다. 그 결과, 보듯이 TPS 측정 결과값이 `219.6/sec` 가 도출되었다. 결과적으로 성능 개선 시도전 `34.1/sec` 에서 `219.6/sec` 로 약 6.4배로 극명하게 개선된 것을 확인할 수 있었다 😎
